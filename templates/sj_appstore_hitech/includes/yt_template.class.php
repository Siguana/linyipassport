...
class YtTemplate {
	...
	// for switch mode view
	/* mode: mobile or windown */
	function getSwitchModes(){
		?>
        <div class="switchmodes-wrap">
            <span class="title-swm">Switch mode views:</span>
            <ul class="nav-swm">
                <li class="window<?php echo ($this->is_mobile==false)?' active':''; ?>"><a title="Window view" href="index.php?switch_modes=2"><span>Window</span></a></li>
                <li class="mobile<?php echo ($this->is_mobile==true)?' active':''; ?>"><a title="Mobile view" href="index.php?switch_modes=1"><span>Mobile</span></a></li>
            </ul>
        </div>
		...
		if ($this->getParam('logoType')=='image'):  
			if($this->getParam('overrideLogoImage')!=''):
				if( strpos($this->getParam('overrideLogoImage'), '/') ){
					...
					if(file_exists('templates/'.$this->template.'/images/'.$this->getParam('sitestyle').'/'.$this->getParam('overrideLogoImage'))){
						$url = 'templates/'.$this->template.'/images/'.$this->getParam('sitestyle').'/'.$this->getParam('overrideLogoImage');
					}else{
						$url = 'templates/'.$this->template.'/images/'.$this->getParam('overrideLogoImage');
					}
				}
			else:
				if(file_exists('templates/'.$this->template.'/images/'.$this->getParam('sitestyle').'/logo.png')){
					$url = 'templates/'.$this->template.'/images/'.$this->getParam('sitestyle').'/logo.png';
				}else{
					$url = 'templates/'.$this->template.'/images/logo.png';
				}			
			endif;
			//echo $url; die();
		?>
	...
	// Render menu
	function getMenu(){
		$menubase = dirname(dirname(__FILE__)) . DS . 'menusys';
		include_once $menubase . DS . 'ytloader.php';
		$browser = new Browser();
	...
	// Get site style
	/* return name of site style(color)*/
	function getStyleSite(){
		$domcolor = new DOMDocument();
		$domcolor->load(JPATH_SITE.DS.'templates'.DS.$this->template.DS.'css'.DS.'color'.DS.'color.xml');
		$sitestyles = $domcolor->getElementsByTagName("sitestyle");
		foreach($sitestyles as $sitestyle){
			foreach($sitestyle->childNodes as $style ){ if( $style->nodeName!='#text'){
					$arr_att = array();
					foreach( $style->childNodes as $att ){ if($att->nodeName!='#text'){
						$arr_att[$att->nodeName] = $att->nodeValue;
					}}
					$this->arr_style[$style->nodeName] = $arr_att;
			}}
		}
		$sitestyle = $this->getParam('sitestyle', 'blue');
		$style_cur = array();
		if( isset($this->arr_style[$sitestyle]) && is_array($this->arr_style[$sitestyle]) ){
			$style_cur = $this->arr_style[$sitestyle];
		}
		return $style_cur ;
	}
	// render possition has attribute group in positions of block
	function renPositionsGroup($position, $type=''){	
		$wbool = 0;
		...
		if ( $position['type'] == 'html' ) {
			$class = "pos-html";
		} elseif ( $position['type'] == 'feature' ) {
			$class = "yt-".strtolower(substr($position['value'], 1));
		}else{
			$class = "";
		}
		if ( $class!="" ) {
		...
		if ( $position['type'] == 'modules' ) { 
			...
			if ( $has_suffix ) {
				$this->renModulePos($position['value'], $position['style'], $elStyle, $class, $position['value'] . '-' . $this->getParam('layoutsuffix'), ($type=='main')?'0':'1');
			} else {
				if ( $doc->countModules($position['value']) ){
					$this->renModulePos($position['value'], $position['style'], $elStyle, $class, '', ($type=='main')?'0':'1');
				}
			}	
			...
			if ( $this->getParam('hideMainContent')=='1'
				...
				&& $this->getParam('switch_modes')=='2' ) {
			?>
            	<span style="display:none">Hide Main content block</span>
            <?php
			} else {
				$this->renComponent($elStyle);
			}
		} elseif ( $position['type'] == 'html' ) { 
			$this->renHTMLPos($position['value'], $elStyle, $class);
		} elseif ( $position['type'] == 'feature' ) {
			$this->renFeaturePos($position['value'],$elStyle, $class);
	...
	function renPositionsNormal($positions, $countModules, $limited, $autosize='1'){
		$doc = &JFactory::getDocument();
		$countend = 0;
		foreach( $positions as $position ){
			...
			if($autosize=='1'){
				$elStyle .= ($position['height']!='')?'height:'.$position['height'].';':'';
				$elStyle .= ($position['width']!='')?'width:'.$position['width'].';':'';
			}
			if($position['type'] == 'modules'){
			...
			if($countend==1 && $countModules>1){ 
				$class .= ($class!='')?" position position-first":'position position-first';
			}elseif($countend==$countModules && $countModules>1){
				$class .= ($class!='')?" position position-last":'position position-last';
			}else{
			...
			$elStyle .= ( $countend == $countModules && $limited=='0' && $countModules>1 )?$this->float2:$this->float1;
			if($elStyle!=""){
				$elStyle = " style='".$elStyle."'";
			}
			...
			if($position['type'] == 'modules'){
				if( $doc->countModules($position['value']) ):
					$this->renModulePos($position['value'], $position['style'], $elStyle, $class);
				endif;
			}elseif($position['type'] == 'html'){
				$this->renHTMLPos($position['value'], $elStyle, $class);
			}elseif($position['type'] == 'feature'){
	...
	function renPositionsContentNoGroup($position){
		...
		if($position['type'] == 'modules'){
			if( $doc->countModules($position['value']) ){
				$this->renModulePos($position['value'], $position['style'], $elStyle);
			}
		}elseif($position['type'] == 'html'){
			$this->renHTMLPos($position['value'], $elStyle);
		}elseif($position['type'] == 'feature'){
			$this->renFeaturePos($position['value'], $elStyle);
		}elseif($position['type']=='message'){ ?>
			<jdoc:include type="message" />
		<?php
		}
	...
	function renModulePos ($positionname, $modulestyle, $elementstyle, $elementclass='', $positionnamesuffix='', $yorn='1' ) {
		if($yorn == '1'){
		?>
		<div id="<?php echo $positionname; ?>"<?php echo $elementstyle; ?><?php echo $elementclass; ?>>
			<!--<div class="yt-position-inner">-->
				<jdoc:include type="modules" name="<?php echo ($positionnamesuffix=='')?$positionname:$positionnamesuffix; ?>" style="<?php echo $modulestyle; ?>" />
			<!--</div>-->
		</div>
		<?php
		}else{ ?>
	...
	function renHTMLPos ($positionvalue, $elementstyle, $elementclass='' ) {
		?>
		<div<?php echo $elementclass; ?><?php echo $elementstyle; ?>>
			<?php echo $positionvalue; ?>
        </div>
		<?php
	}
	//render position with type: feature
	function renFeaturePos ($positionvalue, $elementstyle, $elementclass='' ) {
		?>
		<div<?php echo $elementclass; ?><?php echo $elementstyle; ?>>
			<?php
            if($positionvalue == '@logo'){
                echo $this->getLogo();
            }elseif($positionvalue == '@fontsize'){
                echo $this->getControlFontSize();
            }elseif($positionvalue == '@menu'){
                 $this->getMenu();
            }elseif($positionvalue == '@switch_modes'){
                $this->getSwitchModes();
            }elseif($positionvalue == '@linkFooter'){
                $this->getLinkFooter();
            }elseif($positionvalue == '@copyright'){
                $this->getCopyright();
            }
            ?>
        </div>
	...
	function renComponent ($elementstyle) {
		?>
        <div id="yt_component"<?php //echo $elementstyle; ?> class="masonry-brick">
