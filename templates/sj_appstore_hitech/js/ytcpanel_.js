...
function onChangelayout(obj, idSuffix, element, tmpl_url, tmpl_name, layout_value, wtype) {
	...
	url = tmpl_url+'/includes/cpanel-twidth.php?clayout='+layout_value+'&wtype='+wtype;
...
function onChangeWidthType(element, tmpl_url, tmpl_name, wtype_value, clayout) {
	...
	url = tmpl_url+'/includes/cpanel-twidth.php?clayout='+clayout+'&wtype='+wtype_value;
	$('yt_button_cpanel').setStyle('visibility', 'hidden');
	var c_ajax = new Request({url: url, method:'get', 
		onSuccess: function(result){
				$(element).set('html', result); //alert('dungnv');
